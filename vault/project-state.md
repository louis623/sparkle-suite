...
- The shared Home screenshot reminder now sits inside each homepage hero after its action buttons; its own dark surface, ivory text, and skin-tinted light heading avoid the Halloween dark-on-dark failure. The Dance Floor screenshot reminder uses the same contrast surface. No trade rule, request, account, Live Lineup, extension, or customer data behavior changed.
- Five focused customer-site suites passed 131 tests, including contrast calculations for all 14 appearance presets; TypeScript, targeted lint, JSX parsing, and Vercel's guarded remote build passed. The local `qa:amethyst` link portion could not run because this disposable clone is not an allowlisted dev workbench; the guard was not bypassed.
- Live isolated-browser visual QA passed for Halloween and the default light site on desktop; true 390px Halloween Home and Trade showed no horizontal clipping, and clicking Browse the dance floor retained the correct rep and opened Trade. Mile High Fizz and BlingKitchen custom home layouts displayed the note after their actions. A cropped first mobile screenshot was a Chrome capture artifact, disproven with 390px DevTools geometry and capture. Exact live Home/Trade, FAQ, and representative custom routes returned 200; the affected-route runtime error scan found none. No authenticated session or trade submission was used.
...
- Workspace header now has Report a bug in place of the redundant team chip and a labeled, copyable Live Lineup code. No Chrome extension or live queue synchronization code changed. Rep Message Center has direct reply affordances, clearer team-onboarding participants, search/pagination and Needs reply behavior. Control Center has owner-session-only direct rep messages with private images, separate from broadcasts and Support.
- Supabase migrations `20260923002000` and `20260923010000` were applied and checked. The old September 18 waitlist service-role proposal `20260918180000` was never applied; live service-role read privileges and the September 18 waitlist test made it redundant. It is preserved under `docs/sparkle-suite/operations/retired-migrations/` and removed from the executable migration queue.
- Exact combined-tip build/type check, 57 focused Message Center tests and targeted lint passed. Both Suite domains resolve to the exact deployment; public routes and two customer `/join` routes load; unauthenticated owner/rep APIs deny access. Signed-in visual rep and owner click-through is **not verified** because the supported token-gated synthetic reviewer entry is unavailable to this session. No Louis/customer account or browser session was used. Do not call that acceptance complete until a safe reviewer/operator session is available.
...
- Reviewer-smoke browser preflight was completed in a clean in-app tab. The protected `/start` route correctly redirected to public prelaunch without the token. One normal login attempt using the source-default synthetic reviewer credentials was rejected; production confirms that exact synthetic identity remains active, safely classified as a non-directory demo, and therefore has all-skin access, but its production password is intentionally overridden. No reset, retry, secret extraction, personal-account access, or customer-account access occurred. The signed-in visual picker smoke remains pending the supported token-gated reviewer entry. Do not extract or bypass the token, and do not substitute Louis's account.
...
## September 24, 2026 - Home Dance Floor button label

- The Nic-Nac Home Trade Info button now says `Open Dance Floor` and retains its Dance Floor navigation. Application commit `b2677a08e9ebe86ba726ce343134ad9745f4138f` is live as READY deployment `dpl_D2Um9eCUBEX7WqAX5yf5gMqY6VY1` on both Suite domains; the previous deployment `dpl_59rXL3nvtiBBW9XQ2fvFwKP2STBs` remains available.
- The focused Workspace suite passed 138 tests, TypeScript and the guarded local/Vercel production builds passed, and changed-file lint had zero errors. A broader operator-support suite had two unrelated stale Live Queue/Tools assertion failures. The live landing page and public Dance Floor rendered; signed-in reviewer controls still require an unavailable protected token, so the production button was not visually inspected through an authenticated synthetic session.
