...
- [ ] **Visually verify scoped picker lists with the safe built-in reviewer flow** — authoritative live Sparkle Suite Task List item `76d36fc3-6f90-4c94-99b4-fcfe26c42445` is open for Louis's smoke test. Database policy, server endpoint, and release are verified. Browser authorization and clean-tab preflight completed September 22; `/start` correctly fails closed without the protected token, Vercel will not export that secret, and the source-default synthetic credentials are overridden in production. The synthetic identity itself remains an active non-directory demo with all-skin access. Resume only through the supported token-gated synthetic reviewer entry. Do not extract/bypass the token, reset the account, or use Louis's personal demo account. Confirm one owner sees their private skin, a different rep does not, and an internal reviewer sees all skins.
