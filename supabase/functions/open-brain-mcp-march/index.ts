...
const MCP_ACCESS_KEY = Deno.env.get("MCP_ACCESS_KEY_MARCH")!;
...
server.registerTool(
  ...
  async ({ query, limit, threshold }) => {
    try {
      ...
      const { data, error } = await supabase.rpc("match_thoughts_march", {
...
server.registerTool(
  ...
  async ({ content }) => {
    try {
      ...
      const { data: upsertResult, error: upsertError } = await supabase.rpc("upsert_thought_march", {
        p_content: content,
