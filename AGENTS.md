...
9. Ship through the Smoke-first lane: deploy and verify on Smoke, then one Louis-approved live promote of the verified batch. A push records provenance only. Push is not live. Canonical playbook: Core Memory `skills/sparkle-smoke-ship.md`.
...
**Smoke-first ship lane (locked 2026-09-26):** Approved Suite or Finder application changes are built and verified on Smoke (production twin, staging Supabase, demo accounts). Stack small updates there. Live gets one Louis-approved promote of the verified batch. Same-day live hotfix only when Louis explicitly says so. A merge or push is provenance only, not `vercel --prod`. Canonical playbook: Core Memory `skills/sparkle-smoke-ship.md`. Commit and push approved work without waiting for a separate commit request. Skip commit or push only when Louis says the work is local-only or should not be committed or pushed. Docs-only commits stay undeployed.

**Manual Vercel release policy (August 16, 2026), superseded for the promote step:** Vercel's automatic Git deployment creation stays disabled. Keep the Git link and configured production branch intact. Re-enable automatic creation only with Louis's explicit approval. When Louis says go, the live promote is still one manual deployment of the exact verified batch tip. Then both `https://www.yoursparklesuite.com` and `https://yoursparklesuite.com` must resolve to that deployment.

Do not touch Chrome Web Store settings or local Sparkle Suite Chrome extension code. Treat live queue extension files as protected live-show material. Read `LIVE_EXTENSION_SAFETY.md` before any live extension discussion or handoff. Chrome Web Store publish stays gated: Smoke-installed validation, then Louis, then one Store release. Canonical playbook: Core Memory `skills/sparkle-smoke-ship.md`.

For Smoke verification, logged-in workspace checks, required setup checks, Help & Resources checks, or Nic-Nac UI checks, use `sparkle-suite-production-smoke` when available. Use reviewer-smoke or synthetic sessions instead of Louis's personal account. Live-domain checks happen after Louis approves the batch promote, or names a same-day hotfix.

Default agent and Louis review is Smoke. Exact Smoke URLs and demo logins live in the status file Codex writes (`SPARKLE-SUITE-SMOKE-BUILD-STATUS-*.md`). Never invent them, and never put passwords in git. After a Louis-approved live promote, both `https://www.yoursparklesuite.com` and `https://yoursparklesuite.com` must resolve to that exact deployment. Raw Vercel deployment URLs and `sparkle-suite-demo.vercel.app` are provenance evidence only. Do not hand those URLs to Louis for ordinary review, or describe work as live because they respond. If Louis says something is still wrong on the customer domain, verify the exact live URL he has open before claiming the fix is live.
...
- Smoke review lane: production twin plus staging Supabase and demo accounts. Exact URLs come from the Smoke status file, not from this note.
- Environment model: Smoke for build and verification; live Workspace and Finder only after Louis approves the batch, or names a hotfix. Canonical playbook: Core Memory `skills/sparkle-smoke-ship.md`.
...
Before any live promote, rollback, or domain-alias change:

Smoke verification of the batch comes first unless Louis named a same-day hotfix. Canonical playbook: Core Memory `skills/sparkle-smoke-ship.md`.
...
5. Promote only the verified batch tip Louis approved, then confirm the target aliases point to that exact deployment.
...
- The Smoke path for Louis review, then the live path on `https://www.yoursparklesuite.com` only after the approved promote, or an explicit local URL only when Louis requested local-only work.
