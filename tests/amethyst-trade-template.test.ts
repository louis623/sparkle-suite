...
describe('Amethyst trade page template wiring', () => {
  ...
  it('uses shared route wiring, real listing payloads, and filter wiring on the locked trade export', () => {
    ...
    expect(jsx).toContain('{JOIN_TEAM_HREF && <a {...linkProps(JOIN_TEAM_HREF)} className="hp-header-link">Join Team</a>}')
    expect(jsx).not.toContain('className="hp-header-link hp-header-link-disabled"')
