...
(async()=>{
  ...
  let saved=await run('store.load()');a.equal(saved.token,code);a.equal(saved.workflowVersion,3);a.equal(saved.enabled,true);a.deepEqual(saved.excludedPartyIds,['p2']);
  ...
  a.deepEqual(requests.filter(x=>x.action==='configure').at(-1).excludedPartyIds,['p1','p2']);
  ...
  console.log('PASS: old assigned-code migration, automatic active-tab discovery, automatic parties, filter, toggle, reload and restart recovery');
