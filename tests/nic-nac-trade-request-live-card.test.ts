...
describe('Nic-Nac live trade request card UI wiring', () => {
  ...
  it('defines the live trade request card contract and visible labels', () => {
    ...
    expect(source).toContain("pendingAction === 'approve' ? 'Opening...' : 'Approve'")
    expect(source).toContain("pendingAction === 'reject' ? 'Opening...' : 'Deny'")
    ...
    expect(source).not.toContain('Review to approve')
    expect(source).not.toContain('Review to deny')
